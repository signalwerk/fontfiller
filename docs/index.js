...
function shCollectData(font) {
  ...
  if (THIN_SPACE) {
    ...
    let fallback = findGlyphByUni(font.glyphs.glyphs, 32);
    width.THIN_SPACE = parseInt(fallback.advanceWidth * 0.6);
  }
  ...
  if (NARROW_NO_BREAK_SPACE) {
    ...
    if (THIN_SPACE) {
      ...
      console.log(
        "NARROW_NO_BREAK_SPACE set to 60% width of Glyph 'SPACE' (U+0020)"
      );
      let fallback = findGlyphByUni(font.glyphs.glyphs, 32);
      width.NARROW_NO_BREAK_SPACE = parseInt(fallback.advanceWidth * 0.6);
    }
  }
  ...
  var glyphs = [
    notdefGlyph,
    FIGURE_SPACE_Glyph,
    NO_BREAK_SPACE_Glyph,
    THIN_SPACE_Glyph,
    NARROW_NO_BREAK_SPACE_Glyph
  ];
...
opentype.load(
  "./open-sans-v17-latin_latin-ext/open-sans-v17-latin_latin-ext-regular.woff",
  function(err, font) {
    if (err) {
...
opentype.load(
  "./open-sans-v17-latin_latin-ext/open-sans-v17-latin_latin-ext-700.woff",
  function(err, font) {
    if (err) {
      alert("Font could not be loaded: " + err);
    } else {
